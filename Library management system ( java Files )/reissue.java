...
/*re-issue Book*/
public class reissue implements ActionListener{
	static public int s;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		JLabel lt=new JLabel("Re-issue Book");
		...
		class risearch implements ActionListener{
			
			
			public void actionPerformed(ActionEvent e){
				JButton ri=new JButton("Re-issue");
				try{
					li.bpn.removeAll();
				...
				class reissuebook implements ActionListener{
					public void actionPerformed(ActionEvent e)
					{
						reissue rr=new reissue();
						try{
							int tfine=0;
							String stfine;
							Class.forName("com.mysql.jdbc.Driver");  
								...
								if(tc.getState()==true)
								{
									while(rs.next())
									{
										if(rs.getString(1).equals(tc.getLabel()))
										{
											...
											String query1="update Books set d_o_i=? where b_no="+rs.getString(1);
										    ...
										    String query2="insert into history (action,std_no,bk_no,date,fine) values(?,?,?,?,?)";
										      ...
										      preparedStmt2.setString (1,"Book Re-issue");
				...
				reissuebook rr=new reissuebook();
